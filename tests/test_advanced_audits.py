...
class AdvancedAuditTests(unittest.TestCase):
    def test_build_macro_region_jump_table_counts_new_regions(self) -> None:
        records = pd.DataFrame(
            [
                {"backbone_id": "AA001", "resolved_year": 2014, "country": "USA", "TAXONOMY_family": "Enterobacteriaceae", "TAXONOMY_order": "Enterobacterales"},
                {"backbone_id": "AA001", "resolved_year": 2017, "country": "Germany", "TAXONOMY_family": "Pseudomonadaceae", "TAXONOMY_order": "Pseudomonadales"},
                {"backbone_id": "AA001", "resolved_year": 2018, "country": "Canada", "TAXONOMY_family": "Pseudomonadaceae", "TAXONOMY_order": "Pseudomonadales"},
                {"backbone_id": "AA002", "resolved_year": 2014, "country": "USA", "TAXONOMY_family": "Enterobacteriaceae", "TAXONOMY_order": "Enterobacterales"},
                {"backbone_id": "AA002", "resolved_year": 2017, "country": "USA", "TAXONOMY_family": "Enterobacteriaceae", "TAXONOMY_order": "Enterobacterales"},
            ]
        )
        propensity = pd.DataFrame(
            [
                {"resolved_year": 2017, "country": "Germany", "inverse_upload_weight": 0.8, "rarity_weight": 0.7},
                {"resolved_year": 2018, "country": "Canada", "inverse_upload_weight": 0.5, "rarity_weight": 0.3},
            ]
        )

        result = build_macro_region_jump_table(records, propensity, split_year=2015, test_year_end=2023)
    ...
    def test_build_exposure_adjusted_audits_return_rate_rows(self) -> None:
        ...
        propensity = pd.DataFrame(
            [
                {"resolved_year": 2016, "country": "Germany", "inverse_upload_weight": 0.8, "rarity_weight": 0.7},
                {"resolved_year": 2018, "country": "Japan", "inverse_upload_weight": 0.9, "rarity_weight": 0.8},
                {"resolved_year": 2016, "country": "Canada", "inverse_upload_weight": 0.6, "rarity_weight": 0.5},
                {"resolved_year": 2019, "country": "France", "inverse_upload_weight": 0.4, "rarity_weight": 0.3},
                {"resolved_year": 2020, "country": "Brazil", "inverse_upload_weight": 0.2, "rarity_weight": 0.2},
            ]
        )
        predictions = pd.DataFrame(
            [
        ...
        exposure = build_exposure_adjusted_event_table(records, propensity, split_year=2015, test_year_end=2023)
        audit = build_exposure_adjusted_outcome_audit(predictions, exposure, model_names=["primary"])
    ...
    def test_build_knownness_matched_validation_returns_weighted_row(self) -> None:
        ...
        pred_rows = []
        for i in range(24):
            pred_rows.append({"backbone_id": f"AA{i:03d}", "model_name": "primary", "oof_prediction": 0.8 if i % 2 == 0 else 0.2})
            pred_rows.append({"backbone_id": f"AA{i:03d}", "model_name": "baseline_both", "oof_prediction": 0.6 if i % 2 == 0 else 0.4})
        predictions = pd.DataFrame(pred_rows)

        result = build_knownness_matched_validation(scored, predictions, model_names=["primary", "baseline_both"])

        self.assertIn("__weighted_overall__", set(result["matched_stratum"].astype(str)))
        weighted = result.loc[(result["matched_stratum"] == "__weighted_overall__") & (result["model_name"] == "primary")].iloc[0]
    ...
    def test_build_nonlinear_deconfounding_audit_returns_rows(self) -> None:
        ...
        self.assertEqual(set(result["deconfounding_method"]), {"linear_existing", "quadratic", "stratified"})
    ...
    def test_build_metadata_quality_table_scores_backbones(self) -> None:
        ...
        assembly = pd.DataFrame(
            [
                {"NUCCORE_UID": "1", "ASSEMBLY_Status": "Complete Genome", "ASSEMBLY_coverage": "40x", "ASSEMBLY_SeqReleaseDate": "2020-01-01"},
                {"NUCCORE_UID": "2", "ASSEMBLY_Status": "", "ASSEMBLY_coverage": "", "ASSEMBLY_SeqReleaseDate": ""},
            ]
        )
        biosample = pd.DataFrame(
            [
                {"NUCCORE_UID": "1", "BIOSAMPLE_pathogenicity": "pathogenic", "DISEASE_tags": "sepsis", "ECOSYSTEM_tags": "clinical"},
                {"NUCCORE_UID": "2", "BIOSAMPLE_pathogenicity": "", "DISEASE_tags": "", "ECOSYSTEM_tags": ""},
            ]
        )
        nucc = pd.DataFrame(
            [
                {"NUCCORE_ACC": "seq1", "NUCCORE_Completeness": "complete", "NUCCORE_DuplicatedEntry": "false"},
                {"NUCCORE_ACC": "seq2", "NUCCORE_Completeness": "", "NUCCORE_DuplicatedEntry": "true"},
            ]
        )
        ...
        self.assertLessEqual(float(result.loc[0, "metadata_quality_score"]), 1.0)
        self.assertIn(str(result.loc[0, "metadata_quality_tier"]), {"low", "medium", "high"})

    def test_build_consensus_shortlist_prefers_portfolio_and_fills_from_consensus(self) -> None:
        consensus = pd.DataFrame(
            [
                {"backbone_id": "AA001", "consensus_rank": 1, "consensus_candidate_score": 0.95, "consensus_support_count": 3, "primary_candidate_score": 0.91},
                {"backbone_id": "AA002", "consensus_rank": 2, "consensus_candidate_score": 0.90, "consensus_support_count": 3, "primary_candidate_score": 0.88},
                {"backbone_id": "AA003", "consensus_rank": 3, "consensus_candidate_score": 0.85, "consensus_support_count": 2, "primary_candidate_score": 0.80},
            ]
        )
        portfolio = pd.DataFrame(
            [
                {"backbone_id": "AA002", "portfolio_track": "established_high_risk", "track_rank": 1, "candidate_confidence_tier": "tier_a"},
            ]
        )
        stability = pd.DataFrame(
            [
                {"backbone_id": "AA001", "multiverse_stability_tier": "stable", "multiverse_stability_score": 0.8},
                {"backbone_id": "AA002", "multiverse_stability_tier": "stable", "multiverse_stability_score": 0.9},
            ]
        )
    ...
    def test_build_false_negative_audit_flags_missed_positive_rows(self) -> None:
        ...
        metadata = pd.DataFrame(
            [
                {"backbone_id": "AA001", "metadata_quality_score": 0.30, "metadata_quality_tier": "low"},
                {"backbone_id": "AA002", "metadata_quality_score": 0.90, "metadata_quality_tier": "high"},
            ]
        )
        threshold_flip = pd.DataFrame(
            [
                {"backbone_id": "AA001", "threshold_flip_count": 3, "eligible_for_threshold_audit": True},
    ...
    def test_event_timing_missingness_and_ordinal_outputs(self) -> None:
        records = pd.DataFrame(
            [
                {"backbone_id": "AA001", "sequence_accession": "seq1", "resolved_year": 2014, "country": "USA"},
                {"backbone_id": "AA001", "sequence_accession": "seq2", "resolved_year": 2016, "country": "Germany"},
                {"backbone_id": "AA001", "sequence_accession": "seq3", "resolved_year": 2017, "country": "France"},
                {"backbone_id": "AA001", "sequence_accession": "seq4", "resolved_year": 2018, "country": "Japan"},
                {"backbone_id": "AA002", "sequence_accession": "seq5", "resolved_year": 2014, "country": "USA"},
                {"backbone_id": "AA002", "sequence_accession": "seq6", "resolved_year": 2017, "country": ""},
            ]
        )
        predictions = pd.DataFrame(
            [
        ...
        bounds = build_country_missingness_bounds(records, split_year=2015, test_year_end=2023, threshold=3)
        ordinal = build_ordinal_outcome_alignment(predictions, timing, ordinal_column="spread_severity_bin", model_names=["primary"])
        missingness = build_missingness_sensitivity_performance(predictions, bounds, model_names=["primary"])
        ...
        self.assertGreaterEqual(float(bound_row["optimistic_new_countries"]), float(bound_row["observed_new_countries"]))
    ...
    def test_geographic_duplicate_amr_graph_and_counterfactual_audits(self) -> None:
        records = pd.DataFrame(
            [
                {"backbone_id": "AA001", "sequence_accession": "seq1", "resolved_year": 2014, "country": "USA", "LOCATION_lat": 40.0, "LOCATION_lng": -74.0},
                {"backbone_id": "AA001", "sequence_accession": "seq2", "resolved_year": 2017, "country": "Japan", "LOCATION_lat": 35.0, "LOCATION_lng": 139.0},
                {"backbone_id": "AA002", "sequence_accession": "seq3", "resolved_year": 2014, "country": "USA", "LOCATION_lat": 37.0, "LOCATION_lng": -122.0},
                {"backbone_id": "AA002", "sequence_accession": "seq4", "resolved_year": 2017, "country": "Canada", "LOCATION_lat": 43.0, "LOCATION_lng": -79.0},
            ]
        )
        nucc = pd.DataFrame(
            [
                {"NUCCORE_ACC": "seq1", "NUCCORE_Completeness": "complete", "NUCCORE_DuplicatedEntry": "false"},
                {"NUCCORE_ACC": "seq2", "NUCCORE_Completeness": "", "NUCCORE_DuplicatedEntry": "true"},
                {"NUCCORE_ACC": "seq3", "NUCCORE_Completeness": "complete", "NUCCORE_DuplicatedEntry": "false"},
            ]
        )
        ...
        amr_hits = pd.DataFrame(
            [
                {"NUCCORE_ACC": "seq1", "analysis_software_name": "amrfinderplus", "gene_symbol": "blaA", "drug_class": "BETA-LACTAM"},
                {"NUCCORE_ACC": "seq1", "analysis_software_name": "rgi", "gene_symbol": "blaA", "drug_class": "BETA-LACTAM"},
                {"NUCCORE_ACC": "seq3", "analysis_software_name": "amrfinderplus", "gene_symbol": "tetA", "drug_class": "TETRACYCLINE"},
            ]
        )
        ...
        scored = pd.DataFrame(
            [
                {"backbone_id": "AA001", "spread_label": 1, "n_new_countries": 5, "knownness_score": 0.8, "log1p_member_count_train": 1.0, "log1p_n_countries_train": 0.7, "refseq_share_train": 0.8},
                {"backbone_id": "AA002", "spread_label": 0, "n_new_countries": 1, "knownness_score": 0.2, "log1p_member_count_train": 0.2, "log1p_n_countries_train": 0.1, "refseq_share_train": 0.2},
            ]
        )
        predictions = pd.DataFrame(
            [
        ...
        duplicate = build_duplicate_completeness_change_audit(records, nucc, changes, split_year=2015)
        ...
        self.assertGreaterEqual(float(geo.loc[geo["backbone_id"] == "AA001", "max_jump_distance_km"].iloc[0]), 1000.0)
        self.assertEqual(len(duplicate), 2)
        self.assertEqual(len(amr_uncertainty), 2)
        self.assertEqual(len(mash_graph), 2)
        self.assertEqual(set(counterfactual["selection_mode"]), {"primary_natural", "baseline_natural", "primary_matched_to_baseline", "baseline_matched_to_primary"})
