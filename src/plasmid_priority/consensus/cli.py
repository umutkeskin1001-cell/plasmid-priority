...
def _branch_paths(context_root: Path, data_root: Path) -> dict[str, Path]:
    ...
    return {
        ...
        "geo_predictions_path": data_root / "geo_spread" / "analysis" / "geo_spread_calibrated_predictions.tsv",
        "bio_predictions_path": data_root / "bio_transfer" / "analysis" / "bio_transfer_calibrated_predictions.tsv",
        "clinical_predictions_path": data_root / "clinical_hazard" / "analysis" / "clinical_hazard_calibrated_predictions.tsv",
...
def main(argv: list[str] | None = None) -> int:
    ...
    if paths["root_config_path"].exists():
        paths["branch_config_path"].write_text(paths["root_config_path"].read_text(encoding="utf-8"), encoding="utf-8")
    ...
    with ManagedScriptRun(context, "run_consensus_branch") as run:
        ...
        if load_signature_manifest(
            manifest_path,
            input_paths=[paths["geo_predictions_path"], paths["bio_predictions_path"], paths["clinical_predictions_path"], paths["branch_config_path"]],
        ...
        geo = read_tsv(paths["geo_predictions_path"]) if paths["geo_predictions_path"].exists() else pd.DataFrame()
        bio = read_tsv(paths["bio_predictions_path"]) if paths["bio_predictions_path"].exists() else pd.DataFrame()
        clinical = read_tsv(paths["clinical_predictions_path"]) if paths["clinical_predictions_path"].exists() else pd.DataFrame()
        ...
        results = evaluate_consensus_branch(dataset.table, model_names=model_names, config=context.config)
        summary = build_consensus_model_summary(results)
        predictions = build_consensus_prediction_table(results)
        calibration_summary = build_consensus_calibration_summary(results, scored=dataset.table, config=context.config)
        ...
        used_inventory, unused_inventory, inventory_summary = build_branch_inventory(
            project_root,
            used_paths=[paths["geo_predictions_path"], paths["bio_predictions_path"], paths["clinical_predictions_path"], paths["branch_config_path"]],
            data_root=context.data_dir,
        )
        metrics_payload = {
            "primary_model_name": recommended_primary_model_name or consensus_config.primary_model_name,
        ...
        if "consensus_review_flag" in watchlist.columns:
            watchlist = watchlist.loc[watchlist["consensus_review_flag"].fillna(False).astype(bool)].copy()
