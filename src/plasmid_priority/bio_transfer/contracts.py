"""Branch-level contract checks for the bio transfer task."""

from __future__ import annotations

from collections.abc import Mapping
from typing import Any

from pydantic import BaseModel, ConfigDict, Field

from plasmid_priority.bio_transfer.specs import BioTransferBenchmarkSpec, load_bio_transfer_config
from plasmid_priority.shared.contracts import (
    build_branch_input_contract,
    validate_branch_input_contract,
)


class BioTransferInputContract(BaseModel):
    """Structural contract for bio transfer backbone tables."""

    model_config = ConfigDict(extra="forbid", frozen=True)

    benchmark: BioTransferBenchmarkSpec = Field(default_factory=BioTransferBenchmarkSpec)
    require_temporal_metadata: bool = True
    require_assignment_mode: bool = True
    require_label_consistency: bool = False

    @property
    def required_columns(self) -> tuple[str, ...]:
        return self.benchmark.required_columns


def build_bio_transfer_input_contract(
    config: Mapping[str, Any] | None = None,
    *,
    split_year: int | None = None,
) -> BioTransferInputContract:
    bio_config = load_bio_transfer_config(config)
    benchmark = bio_config.benchmark
    if split_year is not None and int(split_year) != int(benchmark.split_year):
        benchmark = benchmark.model_copy(update={"split_year": int(split_year)})
    return BioTransferInputContract(benchmark=benchmark)


def validate_bio_transfer_input_contract(
    scored,
    *,
    contract: BioTransferInputContract | None = None,
    label: str = "bio transfer input",
) -> None:
    contract = contract or build_bio_transfer_input_contract()
    branch_contract = build_branch_input_contract(contract.benchmark)
    branch_contract = branch_contract.model_copy(update={"require_label_consistency": False})
    validate_branch_input_contract(scored, contract=branch_contract, label=label)
