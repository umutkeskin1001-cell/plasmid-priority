    ...
    for token in tokens:
        if token == "source_balanced":
            dominant_source = np.where(
                eligible["refseq_share_train"].pipe(fill0).to_numpy(dtype=float) >= 0.5,
                "refseq_leaning",
                "insd_leaning",
            )
        ...
        if token == "class_balanced":
            labels = eligible["spread_label"].pipe(int0)
        ...
        if token == "pu_negative_downweight":
            labels = eligible["spread_label"].pipe(int0)
            knownness = _knownness_score_series(eligible).pipe(fill0).clip(lower=0.0, upper=1.0)
        ...
        if token in {"inverse_knownness_weighting", "ipw_balanced"}:
            knownness = _knownness_score_series(eligible).pipe(fill0).clip(0.0, 1.0)
...
def _fit_knownness_residualizer(
    ...
    *,
    ...
    X = working[columns].pipe(fill0).to_numpy(dtype=float)
...
def _select_knownness_residualizer_alpha(
    ...
    *,
    ...
    if "spread_label" not in working.columns:
        return 1.0
    y = working["spread_label"].pipe(int0).to_numpy(dtype=int)
...
def _fit_lightgbm_isotonic_calibrator(
    ...
    *,
    ...
    fit_kwargs = fit_kwargs or {}
    y = train["spread_label"].pipe(int0).to_numpy(dtype=int)
    if len(y) < 8 or len(np.unique(y)) < 2:
        return None
    try:
...
def _oof_lightgbm_predictions_from_eligible(
    ...
    *,
    ...
    y = (
        ...
        else eligible["spread_label"].pipe(int0).to_numpy(dtype=int)
    )
...
def _build_permutation_importance_predictor(
    eligible: pd.DataFrame,
    *,
    columns: list[str],
    fit_kwargs: dict[str, object],
) -> Callable[[pd.DataFrame], np.ndarray]:
    """Fit a single full-data model and return a score-time prediction callable."""
    y = eligible["spread_label"].astype(int).to_numpy(dtype=int)
    model_type = _model_type_name(fit_kwargs)
    sample_weight = _compute_sample_weight(
        eligible,
        mode=_fit_kwarg_mode(fit_kwargs),
    ...
    if model_type == "lightgbm":
        ...
        if _fit_kwarg_str(fit_kwargs, "calibration", "").strip().lower() == "isotonic":
            calibrator = _fit_lightgbm_isotonic_calibrator(eligible, columns, fit_kwargs=fit_kwargs)

        def _predict_lightgbm(score_frame: pd.DataFrame) -> np.ndarray:
            _, X_score = _prepare_feature_matrices(
                eligible,
                score_frame,
                columns,
                fit_kwargs=fit_kwargs,
            ...
            raw = np.asarray(
                model.predict_proba(pd.DataFrame(X_score, columns=columns))[:, 1],
                dtype=float,
            )
            if calibrator is not None:
    ...
    if model_type == "hybrid_stacked":
        ...
        calibrator = _fit_hybrid_isotonic_calibrator(eligible, columns, fit_kwargs=fit_kwargs)

        def _predict_hybrid(score_frame: pd.DataFrame) -> np.ndarray:
            _, X_score = _prepare_feature_matrices(
                eligible,
                score_frame,
                columns,
            ...
            blend_prediction = 0.5 * (logistic_prediction + nonlinear_prediction)
            if calibrator is not None:
                return np.asarray(calibrator.predict(blend_prediction), dtype=float)
            return np.asarray(blend_prediction, dtype=float)
    ...
    if model_type == "pairwise_rank_logistic":
        ...
        def _predict_pairwise(score_frame: pd.DataFrame) -> np.ndarray:
            _, X_score = _prepare_feature_matrices(
                eligible,
                score_frame,
                columns,
            ...
            X_score_scaled = _standardize_apply(X_score, mean, std)
            return _predict_logistic(X_score_scaled, beta)

        return _predict_pairwise
    ...
    def _predict_logistic_family(score_frame: pd.DataFrame) -> np.ndarray:
        _, X_score = _prepare_feature_matrices(
            eligible,
            score_frame,
            columns,
        ...
        X_score_scaled = _standardize_apply(X_score, mean, std)
        return _predict_calibrated(X_score_scaled, beta)

    return _predict_logistic_family
...
def _quantile_binned_codes(values: pd.Series, *, n_bins: int) -> np.ndarray:
    ...
    if int(valid.nunique()) <= max(int(n_bins), 2):
        ...
        result[valid_mask] = valid.astype(float).map(category_map).pipe(int0).to_numpy(dtype=int)
        return result
    ranked = valid.rank(method="average")
    try:
        codes = pd.qcut(
        ...
        result[valid_mask] = 0
        return result
    result[valid_mask] = pd.Series(codes, index=valid.index).pipe(int0).to_numpy(dtype=int)
    return result
...
def build_cmim_feature_selection_table(
    ...
    *,
    ...
    y_codes = eligible["spread_label"].pipe(int0).to_numpy(dtype=int)
    feature_codes = {
...
def annotate_knownness_metadata(scored: pd.DataFrame) -> pd.DataFrame:
    ...
    if not working.empty:
        ...
        knownness_values = to_numeric_series(working["knownness_score"])
...
def _eligible_xy(
    ...
    X = eligible[columns].pipe(fill0).to_numpy(dtype=float)
...
def _knownness_matched_weighted_roc_auc(
    ...
    weights = to_numeric_series(strata["n_backbones"]).pipe(fill0)
    aucs = to_numeric_series(strata["roc_auc"])
...
def _grouped_prediction_weighted_roc_auc(
    ...
    *,
    ...
    if prediction_frame.empty:
        return float("nan")
    merged = scored.loc[scored["spread_label"].notna()].pipe(copy_frame)
    merged = merged.merge(
        prediction_frame[["backbone_id", "oof_prediction"]],
        on="backbone_id",
        how="inner",
    ...
    for group_column in group_columns:
        if group_column not in merged.columns:
            continue
        working = merged.pipe(copy_frame)
        ...
        for group_value in selected_groups:
            frame = working.loc[working[group_column] == group_value].pipe(copy_frame)
    ...
    weights = to_numeric_series(summary["n_backbones"]).pipe(fill0)
    aucs = to_numeric_series(summary["roc_auc"])
    if float(weights.sum()) <= 0.0:
        return float(aucs.mean())
    return float(np.average(aucs, weights=weights))
...
def build_single_model_pareto_screen(
    ...
    *,
    ...
    candidate_family = (
        family.pipe(copy_frame) if family is not None else build_single_model_candidate_family()
    )
    ...
    def _screen_candidate(candidate: dict[str, object]) -> dict[str, object]:
        ...
        prediction_frame = result.predictions.pipe(copy_frame)
        ...
        if prediction_frame.empty:
            ...
            merged_for_score = scored.loc[scored["spread_label"].notna()].pipe(copy_frame)
            ...
            def local_grouped_auc(group_cols: tuple[str, ...]) -> float:
                ...
                for group_col in group_cols:
                    if group_col not in merged_for_score.columns:
                        continue
                    working = merged_for_score.pipe(copy_frame)
...
def fit_full_model_predictions(
    ...
    *,
    ...
    columns = MODULE_A_FEATURE_SETS[model_name]
    working = _ensure_feature_columns(scored, columns)
    train = working.loc[working["spread_label"].notna()].pipe(copy_frame)
    all_rows = working.pipe(copy_frame)
...
def evaluate_model_name(
    ...
    *,
    ...
    eligible = (
        _ensure_feature_columns(scored, columns)
        .loc[scored["spread_label"].notna()]
        .pipe(copy_frame)
    )
    eligible["spread_label"] = eligible["spread_label"].astype(int)
    from plasmid_priority.modeling.knownness import (
...
def _default_model_cache_dir() -> Path:
    env_value = os.environ.get("PLASMID_PRIORITY_MODEL_CACHE_DIR", "").strip()
    if env_value:
        return Path(env_value).expanduser().resolve()
    return (_PROJECT_ROOT / "data" / "tmp" / "model_cache").resolve()


def _model_code_hash() -> str:
    signatures = [
        path_signature_with_hash(Path(__file__), include_file_hash=True),
        path_signature_with_hash(
            Path(__file__).with_name("module_a_support.py"), include_file_hash=True
        ),
    ]
...
def fit_feature_columns_predictions(
    ...
    *,
    ...
    score = _ensure_feature_columns(scored_score, columns).pipe(copy_frame)
...
def build_feature_dropout_audit(
    ...
    *,
    ...
    """Estimate feature importance with permutation drops from a single fitted model."""
    eligible = (
        _ensure_feature_columns(scored, columns)
        .loc[scored["spread_label"].notna()]
        .pipe(copy_frame)
    ...
    if eligible.empty or eligible["spread_label"].nunique() < 2:
        return pd.DataFrame(
            columns=[
                ...
                "brier_without_feature",
                "n_backbones",
                "n_permutation_repeats",
            ],
        )
    fit_kwargs = _model_fit_kwargs(model_name, fit_config)
    predictor = _build_permutation_importance_predictor(
        eligible,
        columns=columns,
        fit_kwargs=fit_kwargs,
    )
    y = eligible["spread_label"].to_numpy(dtype=int)
    full_preds = predictor(eligible)
    ...
    rows = [
        {
            ...
            "feature_rank": 0,
            "importance_method": "single_fit_permutation",
            ...
            "n_permutation_repeats": max(int(n_repeats), 1),
        },
    ]

    _ = n_splits
    _ = _resolve_parallel_jobs(n_jobs, max_tasks=len(columns))
    repeats = max(int(n_repeats), 1)
    ...
    for feature_rank, feature_name in enumerate(columns, start=1):
        ...
        for repeat_idx in range(repeats):
            permuted = eligible.pipe(copy_frame)
            feature_values = permuted[feature_name].to_numpy(copy=True)
            permuted[feature_name] = np.random.default_rng(
                int(seed) + (feature_rank * 1009) + repeat_idx,
            ).permutation(feature_values)
            permuted_preds = predictor(permuted)
            auc_values.append(float(roc_auc_score(y, permuted_preds)))
            ap_values.append(float(average_precision(y, permuted_preds)))
            brier_values.append(float(brier_score(y, permuted_preds)))
        feature_rows.append(
            {
                "model_name": model_name,
                ...
                "importance_method": "single_fit_permutation",
                "roc_auc_without_feature": float(np.mean(auc_values)),
                "average_precision_without_feature": float(np.mean(ap_values)),
                "brier_without_feature": float(np.mean(brier_values)),
                "n_backbones": int(len(eligible)),
                "n_permutation_repeats": repeats,
            },
        )
        ...
        rows.append(
            {
                ...
                "importance_method": feature_row["importance_method"],
                ...
                "n_permutation_repeats": feature_row["n_permutation_repeats"],
            },
        )
...
def run_module_a(
    ...
    *,
    ...
    use_oof_cache = jobs <= 1
    resolved_cache_dir = (cache_dir or _default_model_cache_dir()).resolve()
    fold_cache = FoldCache(resolved_cache_dir)
    matrix_cache = MatrixCache(resolved_cache_dir)
    oof_cache = OOFCache(resolved_cache_dir)
    fold_key, fold_groups, _ = fold_cache.get_or_create(
        y=y,
        split_strategy="repeated_stratified",
        n_splits=n_splits,
        n_repeats=n_repeats,
        seed=seed,
        factory=lambda: _stratified_folds(y, n_splits=n_splits, n_repeats=n_repeats, seed=seed),
    )
    code_hash = _model_code_hash()

    def _evaluate_with_cache(name: str) -> tuple[str, ModelResult]:
        columns = MODULE_A_FEATURE_SETS[name]
        fit_kwargs = _model_fit_kwargs(name)
        matrix_key, matrix_payload = matrix_cache.build_key(
            scored=scored,
            columns=columns,
            preprocessing_config=fit_kwargs,
        )
        if matrix_cache.load(matrix_key) is None:
            matrix_cache.save(matrix_key, matrix_payload)
        oof_key, oof_payload = oof_cache.build_key(
        ...
        cached = oof_cache.load(oof_key) if use_oof_cache else None
        if cached is not None:
            metadata = cached["metadata"]
            predictions = cached["predictions"]
            cached_status = str(metadata.get("status", "ok"))
            if cached_status != "ok":
                cached = None
            else:
                metrics = metadata.get("metrics", {})
                return name, ModelResult(
                    name=name,
                    ...
                    error_message=(
                        ...
                        if metadata.get("error_message") not in (None, "")
                        else None
                    ),
                )
        ...
        if use_oof_cache and result.status == "ok":
            oof_cache.save(
                oof_key,
                key_payload=oof_payload,
    ...
    if jobs > 1:
        ...
        try:
            with limit_native_threads(1):
                with ThreadPoolExecutor(max_workers=jobs) as executor:
                    future_to_name = {
                        executor.submit(_evaluate_with_cache, name): name
            ...
            for name in selected_model_names:
                resolved_name, result = _evaluate_with_cache(name)
                completed[resolved_name] = result
        for name in selected_model_names:
    ...
    primary_eligible = (
        _ensure_feature_columns(scored, primary_columns)
        .loc[scored["spread_label"].notna()]
        .pipe(copy_frame)
    )
