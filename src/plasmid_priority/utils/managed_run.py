"""Managed JSON summaries for pipeline and script runs."""
...
class ManagedScriptRun:
    ...
    _logging_tokens: _LoggingContextTokens | None = field(default=None, init=False, repr=False)
    ...
    def summary_path(self) -> Path:
        return self.logs_dir / f"{self.script_name}_summary.json"
    ...
    @property
    def root(self) -> Path:
        return cast(Path, self.context.root)

    @property
    def logs_dir(self) -> Path:
        return cast(Path, self.context.logs_dir)

    def record_input(self, path: Path, *, include_hash: bool = True) -> None:
        rel_path = relative_path_str(path, self.root)
    ...
    def record_output(self, path: Path) -> None:
        self.output_files_written.append(relative_path_str(path, self.root))
