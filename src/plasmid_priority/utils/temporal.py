"""Temporal helpers used by label factories, contracts, and CV splitters."""
...
def coerce_required_years(
    ...
    *,
    ...
    """Coerce required year metadata and raise on missing/invalid values."""
    ...
    if bool(invalid.any()):
        ...
        message = (
            f"{context}: {int(invalid.sum())} row(s) have invalid "
            f"'{year_column}' values: {examples}"
        )
        raise TemporalMetadataError(message)
