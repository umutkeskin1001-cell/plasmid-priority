...
def format_geo_spread_report_markdown(
    ...
    *,
    ...
    if provenance:
        ...
        recommended_name = str(provenance.get("recommended_primary_model_name", "") or "").strip()
        if recommended_name:
            provenance_lines.append(f"- recommended_primary_model_name: `{recommended_name}`")
    ...
    if not recommended_row.empty:
        recommended_row_item = recommended_row.iloc[0]
        lines.append(f"- recommended_primary_model: `{recommended_row_item['model_name']}`")
        if "selection_score" in recommended_row_item.index:
            recommended_score = _metric_value(recommended_row_item, "selection_score")
