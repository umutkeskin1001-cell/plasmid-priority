...
from plasmid_priority.validation import average_precision, positive_prevalence, roc_auc_score
...
def build_country_upload_propensity(records: pd.DataFrame) -> pd.DataFrame:
    ...
    working["country_clean"] = _clean_text(working.get("country", pd.Series("", index=working.index)))
    working["resolved_year"] = pd.to_numeric(working["resolved_year"], errors="coerce").fillna(0).astype(int)
    ...
    grouped["inverse_upload_weight"] = 1.0 / np.sqrt(grouped["n_records"].clip(lower=1).astype(float))
    grouped["rarity_weight"] = 1.0 - grouped["year_country_share"].clip(lower=0.0, upper=1.0)
    return grouped.sort_values(["resolved_year", "n_records", "country"], ascending=[True, False, True]).reset_index(drop=True)
...
def build_macro_region_jump_table(
    ...
    *,
    ...
    testing = working.loc[(working["resolved_year"] > split_year) & (working["resolved_year"] <= test_year_end)].copy()
    backbone_order = working["backbone_id"].drop_duplicates().tolist()
    result = pd.DataFrame({"backbone_id": backbone_order})

    train_country_pairs = training.loc[training["country_clean"].ne(""), ["backbone_id", "country_clean"]].drop_duplicates()
    test_country_first = (
        testing.loc[testing["country_clean"].ne(""), ["backbone_id", "country_clean", "resolved_year"]]
    ...
    new_country_first = new_country_first.loc[new_country_first["_merge"] == "left_only", ["backbone_id", "country_clean", "resolved_year"]].copy()

    train_region_pairs = training.loc[training["macro_region"].ne(""), ["backbone_id", "macro_region"]].drop_duplicates()
    test_region_pairs = testing.loc[testing["macro_region"].ne(""), ["backbone_id", "macro_region"]].drop_duplicates()
    ...
    new_region_pairs = new_region_pairs.loc[new_region_pairs["_merge"] == "left_only", ["backbone_id", "macro_region"]].copy()

    training["host_family"] = _clean_text(training.get("TAXONOMY_family", pd.Series("", index=training.index)))
    testing["host_family"] = _clean_text(testing.get("TAXONOMY_family", pd.Series("", index=testing.index)))
    train_host_family_pairs = training.loc[training["host_family"].ne(""), ["backbone_id", "host_family"]].drop_duplicates()
    test_host_family_pairs = testing.loc[testing["host_family"].ne(""), ["backbone_id", "host_family"]].drop_duplicates()
    ...
    new_host_family_pairs = new_host_family_pairs.loc[new_host_family_pairs["_merge"] == "left_only", ["backbone_id", "host_family"]].copy()

    training["host_order"] = _clean_text(training.get("TAXONOMY_order", pd.Series("", index=training.index)))
    testing["host_order"] = _clean_text(testing.get("TAXONOMY_order", pd.Series("", index=testing.index)))
    train_host_order_pairs = training.loc[training["host_order"].ne(""), ["backbone_id", "host_order"]].drop_duplicates()
    test_host_order_pairs = testing.loc[testing["host_order"].ne(""), ["backbone_id", "host_order"]].drop_duplicates()
    ...
    new_host_order_pairs = new_host_order_pairs.loc[new_host_order_pairs["_merge"] == "left_only", ["backbone_id", "host_order"]].copy()

    result["n_train_macro_regions"] = result["backbone_id"].map(train_region_pairs.groupby("backbone_id", sort=False).size()).fillna(0).astype(int)
    result["n_test_macro_regions"] = result["backbone_id"].map(test_region_pairs.groupby("backbone_id", sort=False).size()).fillna(0).astype(int)
    result["n_new_macro_regions"] = result["backbone_id"].map(new_region_pairs.groupby("backbone_id", sort=False).size()).fillna(0).astype(int)
    result["new_macro_regions"] = result["backbone_id"].map(
        new_region_pairs.groupby("backbone_id", sort=False)["macro_region"].agg(lambda values: ",".join(sorted(values.astype(str))))
    ).fillna("")
    n_train_countries = result["backbone_id"].map(train_country_pairs.groupby("backbone_id", sort=False).size()).fillna(0).astype(int)
    eligible = n_train_countries.between(1, 3, inclusive="both")
    result["macro_region_jump_label"] = np.where(eligible, result["n_new_macro_regions"].ge(1).astype(float), np.nan)

    result["n_train_host_families"] = result["backbone_id"].map(train_host_family_pairs.groupby("backbone_id", sort=False).size()).fillna(0).astype(int)
    result["n_test_host_families"] = result["backbone_id"].map(test_host_family_pairs.groupby("backbone_id", sort=False).size()).fillna(0).astype(int)
    result["n_new_host_families"] = result["backbone_id"].map(new_host_family_pairs.groupby("backbone_id", sort=False).size()).fillna(0).astype(int)
    result["new_host_families"] = result["backbone_id"].map(
        new_host_family_pairs.groupby("backbone_id", sort=False)["host_family"].agg(lambda values: ",".join(sorted(values.astype(str))))
    ).fillna("")
    result["host_family_jump_label"] = np.where(eligible, result["n_new_host_families"].ge(1).astype(float), np.nan)

    result["n_train_host_orders"] = result["backbone_id"].map(train_host_order_pairs.groupby("backbone_id", sort=False).size()).fillna(0).astype(int)
    result["n_test_host_orders"] = result["backbone_id"].map(test_host_order_pairs.groupby("backbone_id", sort=False).size()).fillna(0).astype(int)
    result["n_new_host_orders"] = result["backbone_id"].map(new_host_order_pairs.groupby("backbone_id", sort=False).size()).fillna(0).astype(int)
    result["new_host_orders"] = result["backbone_id"].map(
        new_host_order_pairs.groupby("backbone_id", sort=False)["host_order"].agg(lambda values: ",".join(sorted(values.astype(str))))
    ).fillna("")
    result["host_order_jump_label"] = np.where(eligible, result["n_new_host_orders"].ge(1).astype(float), np.nan)

    propensity_lookup = propensity_table[["resolved_year", "country", "inverse_upload_weight", "rarity_weight"]].copy() if not propensity_table.empty else pd.DataFrame()
    if not new_country_first.empty and not propensity_lookup.empty:
        ...
        weighted_summary = weighted.groupby("backbone_id", as_index=False).agg(
            weighted_new_country_burden=("inverse_upload_weight", lambda values: float(pd.to_numeric(values, errors="coerce").fillna(0.0).sum())),
            rarity_weighted_new_country_burden=("rarity_weight", lambda values: float(pd.to_numeric(values, errors="coerce").fillna(0.0).sum())),
        )
    ...
    result["weighted_new_country_burden"] = result["weighted_new_country_burden"].fillna(0.0).astype(float)
    result["rarity_weighted_new_country_burden"] = result["rarity_weighted_new_country_burden"].fillna(0.0).astype(float)
    result["n_new_countries_recomputed"] = result["backbone_id"].map(new_country_first.groupby("backbone_id", sort=False).size()).fillna(0).astype(int)
    return result
...
def build_secondary_outcome_performance(
    ...
    *,
    ...
    for model_name in model_names:
        model_rows = predictions.loc[predictions["model_name"] == model_name, ["backbone_id", "oof_prediction"]].copy()
        if model_rows.empty:
            continue
        ...
        rows.append(
            {
                ...
                "status": "ok",
                "n_backbones": int(valid.sum()),
                "spearman_corr": _safe_series_correlation(burden.rank(method="average"), score.rank(method="average")),
                "pearson_corr": _safe_series_correlation(burden, score),
                "mean_weighted_burden_top_25": float(
                    merged.loc[valid].sort_values("oof_prediction", ascending=False).head(25)[burden_column].mean()
                ),
...
def build_count_outcome_alignment(
    ...
    *,
    ...
    for model_name in model_names:
        ...
        if int(valid.sum()) < 5:
            rows.append({"model_name": model_name, "count_column": count_column, "status": "skipped_insufficient_rows"})
            continue
        ...
        rows.append(
            {
                ...
                "status": "ok",
                "n_backbones": int(valid.sum()),
                "spearman_corr": _safe_series_correlation(counts.rank(method="average"), score.rank(method="average")),
                "pearson_corr": _safe_series_correlation(counts, score),
                "mean_count_top_25": float(top25[count_column].mean()) if not top25.empty else float("nan"),
                "median_count_top_25": float(top25[count_column].median()) if not top25.empty else float("nan"),
...
def build_exposure_adjusted_event_table(
    ...
    *,
    ...
    if records.empty:
        return pd.DataFrame()
    timing = build_event_timing_outcomes(records, split_year=split_year, test_year_end=test_year_end)
    macro = build_macro_region_jump_table(records, propensity_table, split_year=split_year, test_year_end=test_year_end)
    ...
    if timing.empty:
        ...
        result = timing[["backbone_id"]].merge(macro[["backbone_id"]], on="backbone_id", how="outer")
    if not timing.empty:
        timing_columns = [
            column
            for column in [
    ...
    result["weighted_new_country_rate_per_year"] = (weighted_burden / float(exposure_years)).astype(float)
    result["rarity_weighted_new_country_rate_per_year"] = (rarity_weighted_burden / float(exposure_years)).astype(float)
...
def build_knownness_matched_validation(
    ...
    *,
    ...
    wide = (
        predictions.loc[predictions["model_name"].isin(model_names), ["backbone_id", "model_name", "oof_prediction"]]
                ...
                metric_row.update(
                    {
                        ...
                        "member_bin": str(frame["member_bin"].iloc[0]) if "member_bin" in frame.columns else "",
                        "country_bin": str(frame["country_bin"].iloc[0]) if "country_bin" in frame.columns else "",
                        "source_bin": str(frame["source_bin"].iloc[0]) if "source_bin" in frame.columns else "",
        ...
        summary_rows.append(
            {
                ...
                "status": "ok",
                "weighted_mean_roc_auc": float(np.average(frame["roc_auc"].astype(float), weights=weights)),
                "weighted_mean_average_precision": float(np.average(frame["average_precision"].astype(float), weights=weights)),
                "n_strata": int(len(frame)),
                "n_backbones": int(frame["n_backbones"].sum()),
                "weighted_positive_prevalence": float(np.average(frame["positive_prevalence"].astype(float), weights=weights)),
...
def build_nonlinear_deconfounding_audit(scored: pd.DataFrame) -> pd.DataFrame:
    ...
    if eligible.empty:
        return pd.DataFrame()
    predictors = eligible[["log1p_member_count_train", "log1p_n_countries_train", "refseq_share_train"]].copy()
    knownness_score = predictors.mean(axis=1)
    rows: list[dict[str, object]] = []
    methods = {
        "linear_existing": eligible.get("H_support_norm_residual", pd.Series(0.0, index=eligible.index, dtype=float)).astype(float),
    ...
    for method_name, residual in methods.items():
        ...
        if n_splits is None:
            rows.append({"deconfounding_method": method_name, "status": "skipped_insufficient_label_variation"})
            continue
        result = evaluate_feature_columns(
            working,
            columns=["T_raw_norm", "H_specialization_norm", "A_raw_norm", "orit_support", residual_column],
...
def build_metadata_quality_table(
    ...
    *,
    ...
    assembly_status = _clean_text(merged.get("ASSEMBLY_Status", pd.Series("", index=merged.index))).str.lower()
    completeness = _clean_text(merged.get("NUCCORE_Completeness", pd.Series("", index=merged.index))).str.lower()
    duplicate_flag = _clean_text(merged.get("NUCCORE_DuplicatedEntry", pd.Series("", index=merged.index))).str.lower().isin(["true", "1", "yes"])
    merged["record_metadata_quality"] = pd.DataFrame(
        {
            ...
            "has_family": _clean_text(merged.get("TAXONOMY_family", pd.Series("", index=merged.index))).ne(""),
            ...
            "low_contig_count": pd.to_numeric(merged.get("typing_num_contigs", pd.Series(0.0, index=merged.index)), errors="coerce").fillna(0.0).between(1, 10),
            "has_typing_gc": pd.to_numeric(merged.get("typing_gc", pd.Series(0.0, index=merged.index)), errors="coerce").fillna(0.0).gt(0),
            "has_typing_size": pd.to_numeric(merged.get("typing_size", pd.Series(0.0, index=merged.index)), errors="coerce").fillna(0.0).gt(0),
            "not_duplicate": ~duplicate_flag,
        }
    ).mean(axis=1)
    rows = (
        merged.groupby("backbone_id", as_index=False)
        .agg(
            ...
            country_coverage_fraction=("country", lambda values: _clean_text(pd.Series(values)).ne("").mean()),
            assembly_status_fraction=("ASSEMBLY_Status", lambda values: _clean_text(pd.Series(values)).ne("").mean()),
            duplicate_fraction=("NUCCORE_DuplicatedEntry", lambda values: _clean_text(pd.Series(values)).str.lower().isin(["true", "1", "yes"]).mean()),
        )
    )
    if not scored.empty and "assignment_confidence_score" in scored.columns:
        ...
        rows["metadata_quality_score"] = rows["metadata_quality_mean"].fillna(0.0).clip(lower=0.0, upper=1.0)
    rows["metadata_quality_tier"] = pd.cut(
        rows["metadata_quality_score"],
        bins=[-0.01, 0.45, 0.70, 1.01],
        labels=["low", "medium", "high"],
    ).astype(str)
    return rows.sort_values(["metadata_quality_score", "n_training_records"], ascending=[False, False]).reset_index(drop=True)
...
def build_consensus_shortlist(
    ...
    *,
    ...
    """Build a reviewer-facing shortlist that prefers curated portfolio rows and fills with consensus candidates."""
    ...
    if not candidate_portfolio.empty:
        ...
        portfolio["portfolio_track"] = portfolio.get("portfolio_track", pd.Series("", index=portfolio.index)).fillna("").astype(str)
        portfolio["track_rank"] = pd.to_numeric(portfolio.get("track_rank", pd.Series(np.nan, index=portfolio.index)), errors="coerce")
        portfolio["_shortlist_track_order"] = portfolio["portfolio_track"].map(track_order_map).fillna(len(track_order_map)).astype(int)
        portfolio["_selection_origin"] = "portfolio"
        sort_columns = [
            column
            for column in ["_shortlist_track_order", "track_rank", "consensus_rank", "primary_model_candidate_score"]
    ...
    shortlist["selection_origin"] = shortlist["_selection_origin"].fillna("consensus_fill").astype(str)
...
def build_false_negative_audit(
    ...
    *,
    ...
    if scored.empty or predictions.empty:
        return pd.DataFrame()
    primary = predictions.loc[predictions["model_name"] == primary_model_name, ["backbone_id", "oof_prediction"]].copy()
    if primary.empty:
        return pd.DataFrame()
    ...
    if {"log1p_member_count_train_rank", "log1p_n_countries_train_rank", "refseq_share_train_rank"} <= set(working.columns):
    ...
    knownness_threshold = float(working["knownness_score"].median()) if working["knownness_score"].notna().any() else math.nan
    ...
    if metadata_quality is not None and not metadata_quality.empty:
        ...
        if meta_columns:
            working = working.merge(metadata_quality[meta_columns].drop_duplicates("backbone_id"), on="backbone_id", how="left")
    if candidate_threshold_flip is not None and not candidate_threshold_flip.empty:
        ...
        if flip_columns:
            working = working.merge(candidate_threshold_flip[flip_columns].drop_duplicates("backbone_id"), on="backbone_id", how="left")
    ...
    train_countries = pd.to_numeric(working.get("n_countries_train", pd.Series(np.nan, index=working.index)), errors="coerce")
    new_countries = pd.to_numeric(working.get("n_new_countries", pd.Series(np.nan, index=working.index)), errors="coerce")
    ...
    for idx in working.index[eligible_threshold & working["threshold_flip_count"].isna()]:
        threshold_labels = [int(new_countries.loc[idx] >= threshold) for threshold in (1, 2, 3, 4, 5)]
        inferred_flip.loc[idx] = float(sum(value != default_status.loc[idx] for value in threshold_labels))
    working.loc[working["threshold_flip_count"].isna(), "threshold_flip_count"] = inferred_flip.loc[working["threshold_flip_count"].isna()]
    working.loc[working["eligible_for_threshold_audit"].isna(), "eligible_for_threshold_audit"] = False
    working["eligible_for_threshold_audit"] = working["eligible_for_threshold_audit"].astype(bool) | eligible_threshold
    ...
    positives["low_knownness_flag"] = positives["knownness_score"].fillna(1.0) <= knownness_threshold if math.isfinite(knownness_threshold) else False
    positives["low_backbone_purity_flag"] = positives.get(
        "backbone_purity_score",
        pd.Series(np.nan, index=positives.index, dtype=float),
    ).fillna(1.0).astype(float) < 0.55
    positives["low_assignment_confidence_flag"] = positives.get(
        "assignment_confidence_score",
        pd.Series(np.nan, index=positives.index, dtype=float),
    ).fillna(1.0).astype(float) < 0.55
    positives["low_metadata_quality_flag"] = positives.get(
        "metadata_quality_score",
        pd.Series(np.nan, index=positives.index, dtype=float),
    ).fillna(1.0).astype(float) < 0.55
    positives["threshold_fragile_flag"] = pd.to_numeric(
        positives.get("threshold_flip_count", pd.Series(np.nan, index=positives.index)),
        errors="coerce",
    ).fillna(0.0).astype(float) >= 2.0
    positives["low_external_host_support_flag"] = positives.get(
        "H_external_host_range_support",
        pd.Series(np.nan, index=positives.index, dtype=float),
    ).fillna(1.0).astype(float) < 0.35
    positives["low_training_members_flag"] = positives.get(
        "member_count_train",
        pd.Series(np.nan, index=positives.index, dtype=float),
    ).fillna(99.0).astype(float) <= 2.0
    ...
    return positives.sort_values(
        ["n_new_countries", "primary_rank", "miss_driver_count"],
        ascending=[False, True, False],
    )[available].head(top_n).reset_index(drop=True)
...
def build_event_timing_outcomes(
    ...
    *,
    ...
    training = working.loc[(working["resolved_year"] <= split_year) & working["country_clean"].ne("")].copy()
    testing = working.loc[
    ...
    new_country_events = new_country_events.loc[new_country_events["_merge"] == "left_only", ["backbone_id", "country_clean", "resolved_year"]].copy()
    new_country_events["event_rank"] = new_country_events.groupby("backbone_id", sort=False).cumcount()
    result = pd.DataFrame({"backbone_id": backbone_order})
    n_train_countries = result["backbone_id"].map(train_country_pairs.groupby("backbone_id", sort=False).size()).fillna(0).astype(int)
    result["n_new_countries_recomputed"] = result["backbone_id"].map(new_country_events.groupby("backbone_id", sort=False).size()).fillna(0).astype(int)
    first_year = new_country_events.groupby("backbone_id", sort=False)["resolved_year"].min()
    third_year = new_country_events.loc[new_country_events["event_rank"] == 2].set_index("backbone_id")["resolved_year"]
    result["time_to_first_new_country_years"] = result["backbone_id"].map(first_year).astype(float) - float(split_year)
    result.loc[result["backbone_id"].map(first_year).isna(), "time_to_first_new_country_years"] = math.nan
    result["time_to_third_new_country_years"] = result["backbone_id"].map(third_year).astype(float) - float(split_year)
    result.loc[result["backbone_id"].map(third_year).isna(), "time_to_third_new_country_years"] = math.nan
    eligible = n_train_countries.between(1, 3, inclusive="both")
    first_delta = result["time_to_first_new_country_years"]
    third_delta = result["time_to_third_new_country_years"]
    result["event_within_1y_label"] = np.where(eligible, ((first_delta <= 1) & first_delta.notna()).astype(float), np.nan)
    result["event_within_3y_label"] = np.where(eligible, ((first_delta <= 3) & first_delta.notna()).astype(float), np.nan)
    result["event_within_5y_label"] = np.where(eligible, ((first_delta <= 5) & first_delta.notna()).astype(float), np.nan)
    result["three_countries_within_3y_label"] = np.where(eligible, ((third_delta <= 3) & third_delta.notna()).astype(float), np.nan)
    result["three_countries_within_5y_label"] = np.where(eligible, ((third_delta <= 5) & third_delta.notna()).astype(float), np.nan)
    severity = pd.Series(np.nan, index=result.index, dtype=float)
    severity.loc[eligible & result["n_new_countries_recomputed"].eq(0)] = 0.0
    severity.loc[eligible & result["n_new_countries_recomputed"].between(1, 2, inclusive="both")] = 1.0
    severity.loc[eligible & result["n_new_countries_recomputed"].between(3, 4, inclusive="both")] = 2.0
...
def build_ordinal_outcome_alignment(
    ...
    *,
    ...
    for model_name in model_names:
        ...
        if int(valid.sum()) < 5:
            rows.append({"model_name": model_name, "ordinal_column": ordinal_column, "status": "skipped_insufficient_rows"})
            continue
        ...
        rows.append(
            {
                ...
                "status": "ok",
                "n_backbones": int(valid.sum()),
                "spearman_corr": _safe_series_correlation(severity.rank(method="average"), score.rank(method="average")),
                "pearson_corr": _safe_series_correlation(severity, score),
                "mean_ordinal_top_25": float(top25[ordinal_column].mean()) if not top25.empty else float("nan"),
...
def build_country_missingness_bounds(
    ...
    *,
    ...
    backbone_order = working["backbone_id"].drop_duplicates().tolist()
    result = pd.DataFrame({"backbone_id": backbone_order})
    train_known_pairs = training.loc[training["country_clean"].ne(""), ["backbone_id", "country_clean"]].drop_duplicates()
    test_known_pairs = testing.loc[testing["country_clean"].ne(""), ["backbone_id", "country_clean", "sequence_accession"]].drop_duplicates()
    ...
    observed_new_pairs = observed_new_pairs.loc[observed_new_pairs["_merge"] == "left_only", ["backbone_id", "country_clean"]].drop_duplicates()
    result["n_countries_train"] = result["backbone_id"].map(train_known_pairs.groupby("backbone_id", sort=False).size()).fillna(0).astype(int)
    training_missing_fraction = training.assign(country_missing=training["country_clean"].eq("")).groupby("backbone_id", sort=False)["country_missing"].mean()
    testing_missing_fraction = testing.assign(country_missing=testing["country_clean"].eq("")).groupby("backbone_id", sort=False)["country_missing"].mean()
    missing_test_records = testing.loc[testing["country_clean"].eq("")].groupby("backbone_id", sort=False).size()
    known_test_records = testing.loc[testing["country_clean"].ne("")].groupby("backbone_id", sort=False).size()
    observed_new = observed_new_pairs.groupby("backbone_id", sort=False).size()
    result["training_country_missing_fraction"] = result["backbone_id"].map(training_missing_fraction).fillna(0.0).astype(float)
    result["testing_country_missing_fraction"] = result["backbone_id"].map(testing_missing_fraction).fillna(0.0).astype(float)
    ...
    result["midpoint_new_countries"] = result["observed_new_countries"] + np.ceil(missing_counts / 2.0).astype(int)
    result["weighted_new_countries"] = np.where(
        (result["observed_new_countries"] > 0) | (missing_counts > 0),
        result["observed_new_countries"].astype(float) * (1.0 + (missing_counts / np.maximum(known_counts, 1))),
        0.0,
    )
    result["eligible_for_country_bounds"] = result["n_countries_train"].between(1, 3, inclusive="both")
    result["label_observed"] = np.where(result["eligible_for_country_bounds"], result["observed_new_countries"].ge(threshold).astype(float), np.nan)
    result["label_midpoint"] = np.where(result["eligible_for_country_bounds"], result["midpoint_new_countries"].ge(threshold).astype(float), np.nan)
    result["label_optimistic"] = np.where(result["eligible_for_country_bounds"], result["optimistic_new_countries"].ge(threshold).astype(float), np.nan)
    result["label_weighted"] = np.where(result["eligible_for_country_bounds"], result["weighted_new_countries"].ge(threshold).astype(float), np.nan)
    return result
...
def build_geographic_jump_distance_table(
    ...
    *,
    ...
    training = working.loc[working["resolved_year"] <= split_year].copy()
    testing = working.loc[(working["resolved_year"] > split_year) & (working["resolved_year"] <= test_year_end)].copy()
    train_groups = {str(backbone_id): frame.copy() for backbone_id, frame in training.groupby("backbone_id", sort=False)}
    test_groups = {str(backbone_id): frame.copy() for backbone_id, frame in testing.groupby("backbone_id", sort=False)}
    rows: list[dict[str, object]] = []
    for backbone_id in working["backbone_id"].astype(str).drop_duplicates().tolist():
        train_frame = train_groups.get(str(backbone_id), pd.DataFrame(columns=training.columns)).copy()
        ...
        for row in test_new.itertuples(index=False):
            ...
            jump_distances.append(
                min(_haversine_km(float(row.lat), float(row.lng), base_lat, base_lng) for base_lat, base_lng in train_coords)
            )
        ...
        rows.append(
            {
                ...
                "long_jump_label": float(int(max_jump >= 2000.0 or interregional >= 1)) if eligible else np.nan,
            }
        )
    return pd.DataFrame(rows)
...
def build_duplicate_completeness_change_audit(
    ...
    *,
    ...
    if not changes.empty:
        ...
        change_payload["change_flag_present"] = _clean_text(_series_or_default(change_payload, "Flag")).ne("") | _clean_text(_series_or_default(change_payload, "Comment")).ne("")
        merged = merged.merge(change_payload[["sequence_accession", "change_flag_present"]].drop_duplicates("sequence_accession"), on="sequence_accession", how="left")
    else:
        merged["change_flag_present"] = False
    completeness = _clean_text(_series_or_default(merged, "NUCCORE_Completeness")).str.lower()
    duplicates = _clean_text(_series_or_default(merged, "NUCCORE_DuplicatedEntry")).str.lower().isin(["true", "1", "yes"])
    ...
    return result.sort_values(["duplicate_completeness_score", "n_training_records"], ascending=[False, False]).reset_index(drop=True)
...
def build_amr_uncertainty_table(
    ...
    *,
    ...
    hits["analysis_software_name"] = _clean_text(_series_or_default(hits, "analysis_software_name")).str.lower()
    ...
    for accession, frame in hits.loc[hits["NUCCORE_ACC"].isin(training_accessions)].groupby("NUCCORE_ACC", sort=False):
        gene_by_tool = {
            tool: set(_clean_text(group["gene_symbol"]).loc[lambda s: s != ""])
            for tool, group in frame.groupby("analysis_software_name", sort=False)
        }
        class_by_tool = {
            tool: set().union(*[_split_field_tokens(value) for value in group.get("drug_class", pd.Series(dtype=object))])
            for tool, group in frame.groupby("analysis_software_name", sort=False)
        }
        ...
        if len(tools) >= 2:
            ...
            class_jaccard = len(class_by_tool[left] & class_by_tool[right]) / max(len(class_union), 1)
        else:
        ...
        accession_rows.append(
            {
                ...
                "gene_agreement_jaccard": float(gene_jaccard) if pd.notna(gene_jaccard) else math.nan,
                "class_agreement_jaccard": float(class_jaccard) if pd.notna(class_jaccard) else math.nan,
                "amr_uncertainty_score": float(
                    1.0 - np.nanmean([gene_jaccard, class_jaccard])
                ) if pd.notna(gene_jaccard) or pd.notna(class_jaccard) else math.nan,
            }
        )
    accession_table = pd.DataFrame(accession_rows)
    if accession_table.empty:
        return accession_table
    merged = training[["backbone_id", "sequence_accession"]].drop_duplicates().merge(accession_table, on="sequence_accession", how="left")
    result = (
        merged.groupby("backbone_id", as_index=False)
        .agg(
            n_training_records=("sequence_accession", "nunique"),
    ...
    return result.sort_values(["mean_amr_uncertainty_score", "n_training_records"], ascending=[True, False]).reset_index(drop=True)
...
def build_mash_similarity_graph_table(
    ...
    *,
    ...
    accession_to_backbone = (
        ...
        .assign(sequence_accession=lambda df: df["sequence_accession"].astype(str), backbone_id=lambda df: df["backbone_id"].astype(str))
    ...
    if not cross_edges.empty:
        cross_pairs = pd.concat(
            [
                cross_edges.rename(columns={"left_backbone": "backbone_id", "right_backbone": "neighbor_backbone"}),
                cross_edges.rename(columns={"right_backbone": "backbone_id", "left_backbone": "neighbor_backbone"}),
            ],
            ignore_index=True,
        ).drop_duplicates()
    ...
    result["mash_graph_within_edge_count"] = result["backbone_id"].map(within_counts).fillna(0).astype(int)
    result["mash_graph_external_neighbor_count"] = result["backbone_id"].map(external_neighbor_count).fillna(0).astype(int)
    result["mash_graph_bridge_fraction"] = (
        (result["mash_graph_edge_count"] - result["mash_graph_within_edge_count"]) / result["mash_graph_edge_count"].clip(lower=1)
    ).astype(float)
    result["mash_graph_novelty_score"] = (1.0 / (1.0 + result["mash_graph_external_neighbor_count"].astype(float))).astype(float)
...
def build_counterfactual_shortlist_comparison(
    ...
    *,
    ...
    if eligible.empty:
        return pd.DataFrame()
    primary = predictions.loc[predictions["model_name"] == primary_model_name, ["backbone_id", "oof_prediction"]].rename(columns={"oof_prediction": "primary_score"})
    baseline = predictions.loc[predictions["model_name"] == baseline_model_name, ["backbone_id", "oof_prediction"]].rename(columns={"oof_prediction": "baseline_score"})
    merged = eligible.merge(primary, on="backbone_id", how="inner").merge(baseline, on="backbone_id", how="inner")
    if merged.empty:
        return pd.DataFrame()
    ...
    merged["matched_budget_stratum"] = merged["matched_source_band"].astype(str) + "|" + merged["matched_knownness_bin"].astype(str)
    ...
    def _summarize(frame: pd.DataFrame, *, label: str, top_k: int) -> dict[str, object]:
        ...
        return {
            ...
            "mean_n_new_countries": float(pd.to_numeric(subset.get("n_new_countries", pd.Series(dtype=float)), errors="coerce").mean()) if len(subset) else np.nan,
            "mean_knownness_score": float(pd.to_numeric(subset["knownness_score"], errors="coerce").mean()) if len(subset) else np.nan,
        }
    ...
    for top_k in top_ks:
        ...
        for stratum, frame in baseline_top.groupby("matched_budget_stratum", sort=False):
            quota = int(len(frame))
            primary_candidates = primary_ranked.loc[primary_ranked["matched_budget_stratum"] == stratum].head(quota)
            if not primary_candidates.empty:
                primary_matched_frames.append(primary_candidates)
        primary_matched = pd.concat(primary_matched_frames, ignore_index=True, sort=False) if primary_matched_frames else pd.DataFrame(columns=merged.columns)
        rows.append(_summarize(primary_matched.sort_values("primary_score", ascending=False), label="primary_matched_to_baseline", top_k=top_k))
        ...
        for stratum, frame in primary_top.groupby("matched_budget_stratum", sort=False):
            quota = int(len(frame))
            baseline_candidates = baseline_ranked.loc[baseline_ranked["matched_budget_stratum"] == stratum].head(quota)
            if not baseline_candidates.empty:
                baseline_matched_frames.append(baseline_candidates)
        baseline_matched = pd.concat(baseline_matched_frames, ignore_index=True, sort=False) if baseline_matched_frames else pd.DataFrame(columns=merged.columns)
        rows.append(_summarize(baseline_matched.sort_values("baseline_score", ascending=False), label="baseline_matched_to_primary", top_k=top_k))
    return pd.DataFrame(rows)
