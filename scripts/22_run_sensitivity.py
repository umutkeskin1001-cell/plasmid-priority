...
def _model_metrics_with_config(
    ...
    *,
    ...
    if fit_config:
        payload.update({str(key): value for key, value in fit_config.items()})
    return payload
...
def _prepare_scored_components(
    ...
    *,
    ...
    assignment_key = (int(split_year), bool(force_fallback_backbones), bool(training_only_backbones))
    ...
    if amr_consensus_cache is not None and consensus_key in amr_consensus_cache:
        ...
        if amr_identity_min is not None:
            filtered_hits = filtered_hits.loc[
                pd.to_numeric(filtered_hits["sequence_identity"], errors="coerce") >= amr_identity_min
            ].copy()
        if amr_coverage_min is not None:
            filtered_hits = filtered_hits.loc[
                pd.to_numeric(filtered_hits["coverage_percentage"], errors="coerce") >= amr_coverage_min
            ].copy()
    ...
    training_canonical = build_training_canonical_table(records, amr_consensus, split_year=split_year)
    ...
    if "backbone_seen_in_training" in working_records.columns:
        working_records["_seen_float"] = working_records["backbone_seen_in_training"].fillna(False).astype(bool).astype(float)
    training_pairs = (
        working_records.loc[(years <= split_year) & working_records["country_clean"].ne(""), ["backbone_id", "country_clean"]]
        .drop_duplicates()
    )
    future_country_first_year = (
        working_records.loc[(years > split_year) & working_records["country_clean"].ne(""), ["backbone_id", "country_clean", "resolved_year_int"]]
    ...
    if not future_country_first_year.empty:
        if training_pairs.empty:
            ...
            training_index = pd.MultiIndex.from_frame(training_pairs[["backbone_id", "country_clean"]])
            future_index = pd.MultiIndex.from_frame(future_country_first_year[["backbone_id", "country_clean"]])
...
def _score_components_for_horizon(
    ...
    *,
    ...
    if not future_country_first_year.empty:
        ...
        n_new_countries = visible_future.loc[visible_future["is_new_country"]].groupby("backbone_id", sort=False).size()
    else:
    ...
    spread_label.loc[eligible] = backbone_table.loc[eligible, "n_new_countries"].ge(DEFAULT_MIN_NEW_COUNTRIES_FOR_SPREAD).astype(int)
    ...
    if "_seen_float" in records.columns:
        ...
        if not future_rows.empty:
            seen_stats = future_rows.groupby("backbone_id", sort=False)["_seen_float"].agg(["sum", "mean"])
            backbone_table["n_test_records_seen_in_training"] = backbone_table["backbone_id"].map(seen_stats["sum"]).fillna(0).astype(int)
            backbone_table["test_seen_in_training_fraction"] = backbone_table["backbone_id"].map(seen_stats["mean"]).fillna(0.0).astype(float)
        else:
...
def _source_balanced_subset(scored: pd.DataFrame, *, seed: int = 42) -> pd.DataFrame:
    ...
    sampled_frames = []
    for _, frame in eligible.groupby("dominant_source", sort=False):
        sampled_frames.append(frame.sample(n=n_per_group, random_state=seed))
...
def _stable_country_new_counts(
    ...
    *,
    ...
    testing = working.loc[(years > split_year) & (years <= test_year_end) & working["country_clean"].ne("")].copy()
    ...
    training_pairs = (
        training.loc[training["country_clean"].isin(stable_countries), ["backbone_id", "country_clean"]]
        .drop_duplicates()
    )
    testing_pairs = (
        testing.loc[testing["country_clean"].isin(stable_countries), ["backbone_id", "country_clean"]]
        .drop_duplicates()
    )
...
def main() -> int:
    ...
    rolling_diagnostic_output_path = context.root / "data/analysis/rolling_assignment_diagnostics.tsv"
    rank_stability_output_path = context.root / "data/analysis/candidate_rank_stability.tsv"
    variant_consistency_output_path = context.root / "data/analysis/candidate_variant_consistency.tsv"
    freeze_output_path = context.root / "data/analysis/prospective_candidate_freeze.tsv"
    annual_freeze_summary_output_path = context.root / "data/analysis/annual_candidate_freeze_summary.tsv"
    ensure_directory(output_path.parent)

    with ManagedScriptRun(context, "22_run_sensitivity") as run:
        for path in (scored_path, backbones_path, amr_hits_path):
        ...
        main_threshold = int(DEFAULT_MIN_NEW_COUNTRIES_FOR_SPREAD)

        variants: dict[str, object] = {
            "default": (scored, default_primary, None),
            "low_coherence_excluded": scored.loc[scored["coherence_score"].fillna(0.0) >= 0.5].copy(),
            "member_count_train_ge_3": scored.loc[scored["member_count_train"].fillna(0).astype(int) >= 3].copy(),
            "alternate_normalization_robust_sigmoid": _rescore_existing_table(
                scored,
        ...
        stable_new = _stable_country_new_counts(records, split_year=2015, test_year_end=2023, min_global_records_per_period=0)
        stable_dense_new = _stable_country_new_counts(records, split_year=2015, test_year_end=2023, min_global_records_per_period=10)
        stable_country_frame = scored.copy()
        stable_country_frame["n_new_stable_countries"] = scored["backbone_id"].map(stable_new).fillna(0).astype(int)
        ...
        stable_dense_country_frame["n_new_stable_dense_countries"] = scored["backbone_id"].map(stable_dense_new).fillna(0).astype(int)
        ...
        for name, variant in variants.items():
            ...
            if len(eligible) < 20 or eligible["spread_label"].nunique() < 2:
                payload[name] = {"skipped": True}
                continue
            payload[name] = _model_metrics_with_config(frame, model_name=model_name, fit_config=fit_config)
        ...
        for split_year in range(2012, 2019):
            for horizon_years in (1, 3, 5, 8):
                ...
                for assignment_mode, training_only in (("all_records", False), ("training_only", True)):
                    ...
                    if assignment_mode == "training_only":
                        freeze_predictions = fit_full_model_predictions(variant_scored, model_name=default_primary).rename(
                            columns={"prediction": "freeze_candidate_score"}
                        )
                        freeze_candidates = (
                        ...
                        annual_freeze_rows.append(
                            {
                                ...
                                "n_positive_candidates": int(freeze_candidates["spread_label"].fillna(0).astype(int).sum()) if not freeze_candidates.empty else 0,
                                "precision_at_25": float(freeze_candidates["spread_label"].fillna(0).astype(float).mean()) if not freeze_candidates.empty else np.nan,
                                "mean_n_new_countries": float(freeze_candidates["n_new_countries"].fillna(0).mean()) if not freeze_candidates.empty else np.nan,
                                "top_backbone_id": str(freeze_candidates.iloc[0]["backbone_id"]) if not freeze_candidates.empty else "",
                            }
                        )
                all_records_result = mode_results.get("all_records", {})
                training_only_result = mode_results.get("training_only", {})
                ...
                if assigned_training_only is not None:
                    assigned_years = pd.to_numeric(assigned_training_only["resolved_year"], errors="coerce").fillna(0).astype(int)
                    future_rows = assigned_training_only.loc[
                        (assigned_years > split_year) & (assigned_years <= window_end)
                    ].copy()
                    if not future_rows.empty:
                        unseen_mask = future_rows["backbone_assignment_rule"].astype(str).eq("unseen_after_training")
                ...
                rolling_diagnostic_rows.append(
                    {
                        ...
                        "roc_auc_delta_training_only_minus_all_records": (
                            ...
                            if all_records_result.get("status") == "ok" and training_only_result.get("status") == "ok"
                            else np.nan
                        ),
                        "average_precision_delta_training_only_minus_all_records": (
                            float(training_metrics["average_precision"] - all_metrics["average_precision"])
                            if all_records_result.get("status") == "ok" and training_only_result.get("status") == "ok"
                            else np.nan
                        ),
                    }
        ...
        consistency_variants = {
            ...
            in {
                ...
                "alternate_normalization_robust_sigmoid",
                "strict_geometric_priority_as_main",
        ...
        freeze_predictions = fit_full_model_predictions(freeze_scored, model_name=default_primary).rename(
            columns={"prediction": "freeze_candidate_score"}
        )
        freeze_candidates = (
        ...
        freeze_candidates["freeze_split_year"] = 2015
        freeze_candidates["evaluation_year_start"] = 2016
        ...
        ordered_columns = leading_columns + [column for column in freeze_candidates.columns if column not in leading_columns]
