...
from plasmid_priority.modeling.ensemble_strategies import (
    ConformalPredictionEnsemble,
    MetaSovereignEnsemble,
    create_meta_sovereign_model,
)
from plasmid_priority.validation.metrics import roc_auc_score, average_precision
...
def main() -> None:
    ...
    best_method = max(ensemble_results.keys(), 
                     key=lambda m: ensemble_results[m]["roc_auc"])
    best_result = ensemble_results[best_method]
    
    # Cross-validation
    print("\n[4/4] Cross-validating best ensemble...")
    final_preds = create_meta_sovereign_model(
        base_preds, y_true, method=best_method
    )["probability"]
    ...
    print(f"Target (83+ AUC): {'✅ ACHIEVED' if best_result['roc_auc'] >= 0.83 else '❌ NOT ACHIEVED'}")
