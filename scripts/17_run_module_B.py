...
def main() -> int:
    ...
    with ManagedScriptRun(context, "17_run_module_B") as run:
        ...
        selected["priority_group"] = selected["priority_index"].ge(high_cut).map({True: "high", False: "low"})

        merged = accessions.merge(selected[["backbone_id", "priority_group"]], on="backbone_id", how="inner")
        ...
        pivot = summary.pivot(index="amr_drug_classes", columns="priority_group", values="prevalence").fillna(0.0)
