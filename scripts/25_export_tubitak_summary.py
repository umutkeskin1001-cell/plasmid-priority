...
def main() -> int:
    ...
    with ManagedScriptRun(context, "25_export_tubitak_summary") as run:
        ...
        cache_key_payload = _cache_key_payload(
            ...
            metadata=pipeline_settings,  # type: ignore
        ...
        if _load_cached_manifest(
            ...
            pipeline_settings=pipeline_settings,  # type: ignore
        ...
        ap_lift = (
            ...
            else tubitak_payload["pr_auc"] - tubitak_payload["prevalence"]  # type: ignore
        )
        ...
        if frozen_row.empty:
            quality_gate_summary["overall_pass"] = all(
                entry["pass"] is True
                for entry in quality_gate_summary["gates"].values()  # type: ignore
            )
        ...
        lines = [
            ...
            f"   Değer            : {_format_metric(tubitak_payload['roc_auc'])}",  # type: ignore
            f"   %95 Güven Aralığı: [{_format_metric(tubitak_payload['roc_auc_ci_lower'])} - {_format_metric(tubitak_payload['roc_auc_ci_upper'])}]",  # type: ignore
            "",
            "2. PRECISION-RECALL ÖZETİ (Average Precision)",
            f"   Değer            : {_format_metric(tubitak_payload['pr_auc'])}",  # type: ignore
            f"   Temel Oran (Pozitif Prevalans) : {_format_metric(tubitak_payload['prevalence'])} "  # type: ignore
            ...
            f"   Değer            : {_format_metric(tubitak_payload['brier_score'])}",  # type: ignore
            f"   Calibration slope      : {_format_metric(tubitak_payload['calibration_slope'])}",  # type: ignore
            f"   Calibration intercept  : {_format_metric(tubitak_payload['calibration_intercept'])}",  # type: ignore
            "",
            "4. FORMAL ANLAMLILIK (Permütasyon Testi)",
            f"   Seçim-düzeltilmiş ROC AUC p-değeri : {_format_pvalue(tubitak_payload['selection_adjusted_permutation_p_roc_auc'])}",  # type: ignore
            f"   Sabit-skor ROC AUC p-değeri      : {_format_pvalue(tubitak_payload['permutation_p_roc_auc'])}",  # type: ignore
            (
            ...
            (
                f"   Spearman ρ       : {_format_metric(tubitak_payload['best_spearman'])} "  # type: ignore
                f"[{_format_metric(tubitak_payload['best_spearman_ci_lower'])} - {_format_metric(tubitak_payload['best_spearman_ci_upper'])}]"  # type: ignore
            ),
            "=================================================================",
            "",
        ...
        if asset_rows:
            ...
            for asset in asset_rows:
                size_text = (
                    str(int(asset["size_bytes"])) if asset.get("size_bytes") is not None else "NA"  # type: ignore
                )
        ...
        for gate_name, gate in quality_gate_summary["gates"].items():  # type: ignore
